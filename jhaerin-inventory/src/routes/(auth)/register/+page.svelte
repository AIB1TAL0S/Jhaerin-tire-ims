...
<div class="bg-background relative flex min-h-screen items-center justify-center px-4 py-12">
	...
	<div class="w-full max-w-md space-y-8">
		...
		<div class="text-center">
			...
			<h1 class="text-foreground text-2xl font-bold tracking-tight">Create Account</h1>
			<p class="text-muted-foreground mt-1 text-sm">Register as a Staff member</p>
		</div>
		...
		<div class="bg-card border-border rounded-xl border p-8 shadow-sm">
			{#if $message?.type === 'success'}
				<div class="mb-6 rounded-lg border border-green-200 bg-green-50 px-4 py-3 text-sm text-green-800 dark:border-green-800 dark:bg-green-950 dark:text-green-200" role="status">
					{$message.text}
					<p class="mt-2">
						<a href="/login" class="text-primary hover:text-primary/80 font-medium underline">Sign in now →</a>
					</p>
				</div>
			{:else}
				...
				<form method="POST" action="?/register" use:enhance class="space-y-5">
					<!-- Hidden role — always Staff for public registration -->
